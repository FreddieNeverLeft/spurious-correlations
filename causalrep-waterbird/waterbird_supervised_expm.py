...
def main():
    ...
    parser = argparse.ArgumentParser(description='Waterbird Supervised')
    ...
    parser.add_argument('-d', '--dataset', default="CUB")
    parser.add_argument('-s', '--shift_type', default="confounder")
    ...
    test_data = None
    test_loader = None
    train_data, val_data, test_data = prepare_data(flags, train=True)


    loader_kwargs = {'batch_size':flags.batch_size, 'num_workers':4, 'pin_memory':True}
    train_loader = train_data.get_loader(train=True, reweight_groups=flags.reweight_groups, **loader_kwargs)
    ...
    #     print(batch)
    # print(train_data._group_array)
    # for x,y,g in train_data.dataset:
    #     print(x.shape)

    train_x = torch.cat([x.flatten()[None,:] for x,y,g in train_data.dataset], dim=0)
    train_y = torch.Tensor([y for x, y, g in train_data.dataset])
    train_g = torch.Tensor([g for x, y, g in train_data.dataset])
    print(train_x.shape, train_y.shape, train_g.shape)

    test_x = torch.cat([x.flatten()[None,:] for x, y, g in test_data.dataset], dim=0)
    test_y = torch.Tensor([y for x, y, g in test_data.dataset])
    test_g = torch.Tensor([g for x, y, g in test_data.dataset])
    print(test_x.shape, test_y.shape, test_g.shape)
    ...
    for restart in range(flags.n_restarts):
        ...
        #     full_labels_df['noisy_target'] = np.logical_xor(full_labels_df['true_target'], npr.binomial(n=1, p=noise,
        #                                                                                                 size=len(
        #                                                                                                     full_labels_df[
        #                                                                                                         'true_target'])))
        ...
        # train_y_df = data['train_data']
        # test_y_df = data['train_loader']
        #
        # print("train spurious corr:\n", train_y_df.corr())
        # print("test spurious corr:\n", test_y_df.corr())

        # train_data = torch.stack(
        #     [celeba_dataset[idx][:, ::4, ::4].reshape(-1, 3 * 16 * 16) for idx in np.array(train_y_df['index'])])
        #
        # test_data = torch.stack(
        #     [celeba_dataset[idx][:, ::4, ::4].reshape(-1, 3 * 16 * 16) for idx in np.array(test_y_df['index'])])
        #
        ...
        envs = [
            {
                'images': (train_x.float()),
                'labels': train_y[:, None].float(),
                'spurious_target': train_g[:, None]
            },
            {
                'images': (test_x.float()),
                'labels': test_y[:, None].float(),
                'spurious_target': test_g[:, None]
            }
        ]
        ...
        optimizer_nll = optim.Adam(list(mlp._tvaez.parameters()) + list(mlp.finallayer.parameters()), lr=1. * flags.lr,
        ...
        for step in range(flags.steps):
            ...
            for i in range(len(envs)):
                ...
                env['skl_predacc'] = mean_accuracy_np(pred.predict(features.detach().cpu().numpy()),
                                                      labels.detach().cpu().numpy().T[0])
